...
export default function Dashboard({
    ...
    const greeting  = flash.isFirstLogin
        ? `Welcome, ${firstName}!`
    ...
    const maxTrend = Math.max(...trendData.map((item) => item.value), 0);

    const statCards = [
        {
            label: 'Total payroll',
            value: `₱${Number(stats?.totalPayroll ?? 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            delta: '4.2%',
            trend: 'down',
            icon:  Banknote,
    ...
    const anomalies = (needsReview ?? [])
        ...
        .slice(0, 3)
        .map((item) => {
            const employeeName  = [item.employee?.first_name, item.employee?.last_name].filter(Boolean).join(' ');
            ...
            return {
                ...
                amount:   amountValue >= 0
                    ...
                    : `-₱${Math.abs(amountValue).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
            };
        });
            ...
            <main className="flex-1 space-y-6 p-6">
                ...
                <div className="relative overflow-hidden rounded-2xl bg-[#16241c] p-6">
                    ...
                    <div className="relative flex flex-wrap items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="font-['Space_Grotesk'] text-sm text-white/55">
                                Payroll performance and what needs your attention today.
                            </p>
                        </div>
                        ...
                        <button
                            ...
                            className="flex items-center gap-2 rounded-full bg-[#b98a2e] px-5 py-2.5 font-['Space_Grotesk'] text-sm font-medium text-[#16241c] hover:bg-[#c99a3e]"
                        >
                    ...
                    {statCards.map((stat) => (
                        ...
                        >
                            <div className="flex items-center justify-between">
                                ...
                                <span className={`flex items-center gap-0.5 font-['IBM_Plex_Mono'] text-xs font-medium ${stat.trend === 'up' ? 'text-[#16A34A]' : 'text-[#B98A2E]'}`}>
                                    {stat.trend === 'up'
                ...
                <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
                    ...
                    <div className="rounded-2xl bg-white p-6 ring-1 ring-[#14172B]/5 dark:bg-white/5 dark:ring-white/10">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <p className="mt-0.5 font-['Space_Grotesk'] text-xs text-[#14172B]/50 dark:text-white/50">
                        ...
                        {trendData.length === 0 ? (
                            <div className="flex h-40 items-center justify-center">
                                <p className="font-['Space_Grotesk'] text-sm text-[#14172B]/40 dark:text-white/40">
                                    No payroll trend data available.
                                </p>
                            </div>
                        ) : (
                            <div className="mt-16 flex h-50 items-end gap-2 rounded-lg bg-gray-100">
                                {trendData.map((item) => (
                                    <div key={item.id} className="flex flex-1 flex-col items-center gap-2">

                                        <span className="font-['IBM_Plex_Mono'] text-[11px] text-[#14172B]/50 dark:text-white/90">
                                        ...
                                        <div className="relative flex h-32 w-full items-end justify-center">
                                            ...
                                            <div
                                                ...
                                                style={{ height: maxTrend > 0 ? `${(item.value / maxTrend) * 100}%` : '0%' }}
                                            />
                                        </div>
                    ...
                    <div className="rounded-2xl bg-white p-6 ring-1 ring-[#14172B]/5 dark:bg-white/5 dark:ring-white/10">
                        <div className="flex items-center justify-between">
                            ...
                            <span className="rounded-full bg-[#F5A524]/15 px-2 py-0.5 font-['Space_Grotesk'] text-xs font-medium text-[#B98A2E]">
                        ...
                        <div className="mt-4 space-y-3">
                            {anomalies.length === 0 ? (
                                <div className="rounded-xl border border-dashed border-[#14172B]/15 px-3 py-5 text-center font-['Space_Grotesk'] text-sm text-[#14172B]/45 dark:border-white/10 dark:text-white/45">
                                ...
                                anomalies.map((a) => (
                                    ...
                                    >
                                        <div className="min-w-0">
                                            <p className="truncate font-['Space_Grotesk'] text-sm font-medium text-[#14172B] dark:text-white">
                                                {a.employee}
                                            </p>
                                            <p className="font-['Space_Grotesk'] text-xs text-[#14172B]/45 dark:text-white/45">
                                                {a.role}
                                            </p>
                                            <p className="mt-1 font-['Space_Grotesk'] text-xs leading-5 text-[#14172B]/65 dark:text-white/65">
                                                {a.issue}
                                            </p>
                                        </div>
                ...
                <div className="rounded-2xl bg-white p-6 ring-1 ring-[#14172B]/5 dark:bg-white/5 dark:ring-white/10">
                    <div className="flex items-center justify-between">
                        ...
                        <a href="#" className="font-['Space_Grotesk'] text-xs font-medium text-[#16241c] hover:underline">
                            View all
                        </a>
                    </div>
                    ...
                    <div className="mt-4 overflow-x-auto">
                        <table className="w-full text-left text-sm">
                            <thead>
                                <tr className="font-['Space_Grotesk'] text-xs text-[#14172B]/45 dark:text-white/45">
                            ...
                            <tbody>
                                {payRuns.map((run) => (
                                    <tr key={run.id} className="border-t border-[#14172B]/6 dark:border-white/10">
                                        ...
                                        <td className="py-3 font-['Space_Grotesk'] text-[#14172B] dark:text-white">
                                            {new Date(run.period_start).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                            {' - '}
                                            {new Date(run.period_end).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                                        </td>

                                        <td className="py-3 font-['IBM_Plex_Mono'] text-[#14172B] dark:text-white">
                                            ₱{Number(run.total_net ?? 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </td>

                                        <td className="py-3">
                                            <span className={`rounded-full px-2.5 py-1 font-['Space_Grotesk'] text-xs font-medium ${statusStyle[run.status] ?? 'bg-gray-500/15 text-gray-500'}`}>
                                                {run.status}
                                            </span>
                                        </td>
