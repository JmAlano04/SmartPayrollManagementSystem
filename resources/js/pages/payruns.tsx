    ...
    */
    ...
    const filteredPayRuns = useMemo(() => {
        ...
        return payRuns.filter((payrun) => {
            ...
            const payDate =
                String(payrun.pay_date ?? '').toLowerCase();

            const grossPay =
                String(payrun.gross_pay ?? '').toLowerCase();

            const netPay =
                String(payrun.net_pay ?? '').toLowerCase();

            const employeesCount =
                String(payrun.employees_count ?? '').toLowerCase();

            const name =
                String(payrun.name ?? '').toLowerCase();

            // Database uses "pending", UI displays it as "Draft"
            const statusText =
